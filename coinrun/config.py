...
class ConfigSingle(object):
    ...
    def __init__(self):
        ...
        type_keys.append(('num-levels-to-collect', 'num_levels_to_collect', int, 5))
        type_keys.append(('save-dir', 'save_dir', str, 'video_data'))
        type_keys.append(('level-timeout', 'level_timeout', int, 1000))
        bool_keys.append(('collect_data', 'collect_data'))

        # Agent Reward Config
        type_keys.append(('air-control', 'air_control', float, 0.15))
        type_keys.append(('bump-head-penalty', 'bump_head_penalty', float, 0.0))
        type_keys.append(('die-penalty', 'die_penalty', float, 0.0))
        type_keys.append(('kill-monster-reward', 'kill_monster_reward', float, 5.0))
        type_keys.append(('jump-penalty', 'jump_penalty', float, 0.0))
        type_keys.append(('squat-penalty', 'squat_penalty', float, 0.0))
        type_keys.append(('jitter-squat-penalty', 'jitter_squat_penalty', float, 0.0))

        # Should the agent's velocity be painted in the upper left corner of observations.
        # 1/0 means True/False
        # PAINT_VEL_INFO = -1 will default to 0
        ...
        # Perform evaluation with all levels sampled from the test set
